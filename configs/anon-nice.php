...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  $userid = $_SESSION['id'];
  $MessageBody = filter_var($_POST['NiceMess'], FILTER_SANITIZE_STRING);
  ...
  if (strlen($MessageBody) > 255 || strlen($MessageBody) < 1) { } else {
    $NiceMesstStmt = $db->prepare('INSERT INTO anonymous_messages(body, type, mess_date, from_user, to_user) VALUES (:messbody, 1, NOW(), :userid, :toid)');
    $NiceMesstStmt->execute(array(
