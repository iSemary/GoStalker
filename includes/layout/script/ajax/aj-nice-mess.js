$(document).on('submit', '#nice-form', function(e) {
  e.preventDefault();
  if ($('#emojiPickNice').val().length < 1 || $('#emojiPickNice').val().length > 255) {
      ...
      $.ajax({
          url: "../gostalker/configs-nice.php",
          type: "POST",
          ...
          success: function(response) {
              if (response = "VALID") {
                  ...
                  $('#main-wait').hide();
                  $('#post-success').html('Your nice message has been sent.');
                  $('#main-success').show();
